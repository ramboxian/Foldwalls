...
客户端使用 `library.json` 保存轻量的离线目录、收藏和下载状态，媒体文件保存在应用资料库的 `Library` 目录。启动时只请求目录；列表缩略图在可见时缓存；Hero 和悬停播放使用轻量视频预览；原始媒体仅在用户点击应用时下载，并在下载后校验文件大小再原子写入。已有本地素材会按云端 ID 或原始文件名合并，不会重复下载。
