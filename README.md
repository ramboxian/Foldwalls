...
- 通用、播放、显示器和资料库四类设置
- 4K 原文件与轻量预览缩略图分离
- Sanity 云端目录同步（项目 `3huccpow`，`production` 数据集）与 Cloudflare R2 双源媒体分发
- 云端缩略图按需缓存，原图与视频仅在用户应用时下载
- 首页 Hero 首次完整缓存原片后自动播放，之后直接读取本地原片；其他视频卡片在悬停时加载轻量预览
...
应用通过 `SanityCatalogProvider` 获取已发布壁纸的结构化元数据；媒体迁移期间 Sanity Asset CDN 与 Cloudflare R2 并行保存图片、视频与缩略图。每条壁纸都可独立切换为 R2 优先，读取或下载失败时自动回退 Sanity；旧版客户端继续使用原来的 Sanity 字段。数据库中只保存标题、分类、标签、尺寸、文件地址等元数据，不保存媒体二进制。

客户端使用 `library.json` 保存轻量的离线目录、收藏和下载状态，媒体文件保存在应用资料库的 `Library` 目录。启动时只请求目录；列表缩略图在可见时缓存；Hero 首次下载并校验原片后从永久缓存播放，悬停卡片使用轻量预览；其他原始媒体仅在用户点击应用时下载。Hero 已缓存的原片会在应用壁纸时直接复用，已有本地素材按云端 ID 或原始文件名合并，不会重复下载。
